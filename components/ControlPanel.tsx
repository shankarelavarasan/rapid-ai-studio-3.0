...
export const ControlPanel: React.FC<ControlPanelProps> = ({ addTrack, onAiCompose, isLoading, initializationError, bpm, playSampleNow }) => {
    ...
    const renderContent = () => {
        ...
        if (initializationError) {
             return (
                <div className="flex-grow flex items-center justify-center p-4 bg-red-900/50 rounded-lg">
                    <div className="text-center">
                        <p className="font-semibold text-red-300">Audio Engine Failed</p>
                        <p className="mt-2 text-sm text-red-200">{initializationError}</p>
                    </div>
                </div>
            );
        }
                            ...
                            >
                                 {isAnalyzing ? (
                                    <>
                                        ...
                                        <p className="mt-3 text-sm text-gray-400">AI is analyzing your audio...</p>
                                    </>
                                 ) : (
                                    <>
                                        <UploadIcon />
                                        <p className="mt-2 text-sm text-gray-400 text-center">
                                            {mode === 'instrument' 
                                                ? `Upload audio to recreate with ${selectedInstrument}`
